...
function AgentCard({ result }: { result: AgentResult }) {
    ...
    return (
        ...
        >
            ...
            {/* Status badge */}
            {installed ? (
                <span className="inline-flex self-start py-0.5 px-2 text-[0.7rem] font-bold text-emerald-700 bg-emerald-100/15 rounded">
                    ✓ Installed
                </span>
            ) : (
                <span
...
export function Agents() {
    ...
    return (
        <main className="flex flex-col items-center min-h-screen pt-12 px-8 pb-20 box-border">
            ...
            {/* ---- Error ---- */}
            {status === "error" && error && (
                <div className="mt-6 py-3 px-5 text-sm text-amber-900 bg-amber-100/15 border border-amber-300/25 rounded-[10px] max-w-[520px] text-center">
                    <span>⚠️</span> {error}
                </div>
            )}
